...
from pandas import DataFrame, Series, Index, Period, read_excel
...
def read_train_test_from_df(train: DataFrame, test: DataFrame, target: str = "class"):
    labels = sorted(train[target].unique())
    trnY = train[target].values
    trnX = train.drop(columns=[target]).values
    tstY = test[target].values
    tstX = test.drop(columns=[target]).values
    vars = [c for c in train.columns if c != target]
    return trnX, tstX, trnY, tstY, labels, vars


def read_train_test_from_files(
    train_fn: str, test_fn: str, target: str = "class"
) -> tuple[ndarray, ndarray, array, array, list, list]:
    train: DataFrame = read_excel(train_fn, index_col=None)
    labels: list = list(train[target].unique())
    labels.sort()
    trnY: array = train.pop(target).to_list()
    trnX: ndarray = train.values

    test: DataFrame = read_excel(test_fn, index_col=None)
...
def plot_evaluation_results(model, trn_y, prd_trn, tst_y, prd_tst, labels: ndarray, proba_trn=None, proba_tst=None) -> ndarray:
    evaluation: dict = {}

    for key in CLASS_EVAL_METRICS:
        # Para ROC AUC, usar probabilidades se disponíveis
        if key == 'roc_auc' and proba_trn is not None and proba_tst is not None:
            evaluation[key] = [
                CLASS_EVAL_METRICS[key](trn_y, proba_trn),
                CLASS_EVAL_METRICS[key](tst_y, proba_tst),
            ]
        else:
            evaluation[key] = [
...
#sfrom statsmodels.tsa.seasonal import DecomposeResult, seasonal_decompose
